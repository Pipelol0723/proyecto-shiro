...
9. **Packaging Tauri** 🟡 en progreso — bootstrap Tauri 2.0 en `packages/desktop/src-tauri/` con tray icon, close-to-tray, single-instance, plugins fs/shell/dialog registrados pero sin permisos concedidos al webview (runtime-denied real vía ACL de capabilities — ver enmienda ADR 0024 §5). **Sidecar del core-host**: empaquetado con ncc+pkg (`build:sidecar`), `better-sqlite3.node` cargado vía `nativeBinding`, lanzado/matado desde Rust con configs por env var. Ver [ADR 0024](docs/adr/0024-packaging-tauri-windows-sidecar.md). Pendiente: healthcheck wizard (+ secrets en runtime), modo overlay opcional, auto-updater, workflow CI release.
