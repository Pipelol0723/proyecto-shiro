...
8. **Avatar Live2D** 🟡 _en progreso_ — render de Hiyori vía `pixi-live2d-display-lipsyncpatch` (PixiJS v7, Cubism Core **4.2.2** — el SDK 5/Core 6 crashea el renderer) dentro de `<Avatar>`, con fallback automático al orbe (PRs #51-#53). **Lip-sync** de la boca con la voz del TTS (Web Audio → `ParamMouthOpenY`); idle off por default porque sus motions compiten con el lip-sync. Faltan expresiones por emoción (PR #4). Ver [ADR 0021](docs/adr/0021-avatar-live2d-pixi-display-fallback-orbe.md).
